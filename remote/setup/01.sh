...
USERNAME=csflashcards
# Prompt to enter a password for the PostgreSQL csflashcards user (rather than hard-coding # a password in this script).
read -p "Enter password for csflashcards DB user: " DB_PASSWORD
...
# Set up the csflashcards DB and create a user account with the password entered earlier.
sudo -i -u postgres psql -c "CREATE DATABASE csflashcards"
sudo -i -u postgres psql -d csflashcards -c "CREATE ROLE csflashcards WITH LOGIN PASSWORD '${DB_PASSWORD}'"
# Add a DSN for connecting to the csflashcards database to the system-wide environment
# variables in the /etc/environment file.
echo "CSFLASHCARDS_DB_DSN='postgres://csflashcards:${DB_PASSWORD}@localhost/csflashcards'" >> /etc/environment
